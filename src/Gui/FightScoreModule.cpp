...
AoL::Gui::FightScoreModule::FightScoreModule(std::wstring const &text,
...
AoL::Gui::FightScoreModule::~FightScoreModule()
{
  for (irr::u32 i = 0; i < _nbButtonsHolder; ++i)
    delete _buttonsHolder[i];
}

inline std::wstring const		&AoL::Gui::FightScoreModule::getText() const
{
  return (_text);
}

inline std::string const		&AoL::Gui::FightScoreModule::getPicturePath() const
{
  return (_picturePath);
}

inline std::vector<AoL::Gui::ButtonHolder *>	&AoL::Gui::FightScoreModule::getButtonsHolder()
{
  return (_buttonsHolder);
}

inline irr::u32				AoL::Gui::FightScoreModule::getNbButtonsHolder() const
{
  return (_nbButtonsHolder);
}

inline AoL::Gui::GUITypeId	AoL::Gui::FightScoreModule::getType() const
{
  return (_type);
}

inline bool			AoL::Gui::FightScoreModule::isHide() const
{
  return (_hide);
}

void		     			AoL::Gui::FightScoreModule::dump() const
{
